...
/*   Updated: 2024/08/29 15:47:53 by jeguerin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	try_move(t_game *game, double move_x, double move_y)
{
	...
	if (!is_wall(game, new_x, game->player.y)) // Move along X if no collision
	{
		game->player.x = new_x;
	}
	if (!is_wall(game, game->player.x, new_y)) // Move along Y if no collision
	{
		game->player.y = new_y;
	}
}
...
void	is_action(t_game *game)
{
	...
	movement_speed = game->player.speed * 0.05; // Adjust value to reduce speed
	is_moving = 0;
	...
	if (is_moving)
	{
		game->frame_count++; // Adjust the multiplier (10) for stronger effect
...
//     if (dx != 0) {
//         rotate_player(game, -dx * 0.003);  // Ajuster la sensibilité de 
//				la souris si nécessaire
//     }
//     return (0);
// }
